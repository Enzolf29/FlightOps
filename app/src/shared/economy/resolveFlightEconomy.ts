...
export interface ResolveFlightEconomyInput {
  ...
  /** Surtaxe "petit aéroport" côté départ, basée sur le nombre de destinations connues (voir
   * computeAirportSurcharge), ex. 0.15 pour +15 %. */
  airportSurchargeFraction: number
  /** Surtaxe propre à cette ligne précise, basée sur sa part d'observations par rapport aux autres
   * lignes au départ du même aéroport (voir computeRouteSurcharge) — vient s'ajouter à la surtaxe
   * aéroport pour refléter qu'un sens peu confirmé (LFRB → LFPG) coûte plus cher que le sens
   * inverse, bien plus souvent observé. */
 ...
 * qui en découle, avant réservation SimBrief — le nombre de passagers obtenu est ensuite injecté
 * dans l'URL de dispatch SimBrief (pax) pour que le plan généré reflète la demande.
 */
export function resolveFlightEconomy(
  ...
  const referenceTicketPriceEur =
    computeReferenceTicketPriceEur(input.referenceFareModel, distanceNm) *
    (1 + input.airportSurchargeFraction + input.routeSurchargeFraction)
